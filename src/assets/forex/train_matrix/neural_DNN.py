...
def run_model(df, asset, lookback, n_components, training_cols, model_num, learning_rate=0.001, batch_size=128, epochs=300, seed=42):
    ...
    # Use ModelWrapper to wrap the trained neural network model
    ...
    log_loss_value_calibrated = log_loss(np.argmax(y_test, axis=1), y_pred_proba_calibrated)
    print(f'Log Loss (Calibrated): {log_loss_value_calibrated}')

    # Calculate Brier score for the calibrated model
    brier_score_calibrated = brier_score_loss(np.argmax(y_test, axis=1), y_pred_proba_calibrated)
    ...
    return model, y_pred_proba_calibrated, y_test