...
def get_volume_bars(df, volume_threshold):
    ...
    volume_bars_df['Daily_Returns'] = (volume_bars_df['Close'].pct_change() *100)
...
def get_dollar_bars(df, dollar_threshold):
    ...
    dollar_bars_df = pd.DataFrame(dollar_bars).set_index('Datetime')

    #scaler = StandardScaler()

   # dollar_bars_df['Date'] = dollar_bars_df.index
