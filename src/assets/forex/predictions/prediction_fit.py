...
def make_predictions(symbol, new_data: pd.DataFrame) -> pd.Series:
    ...
    clf = joblib.load(f'models/{symbol}_model/random_forest_model_up_{symbol}.pkl')
    pca = joblib.load(f'models/{symbol}_model/pca_transformation_up_{symbol}.pkl')
    scaler = joblib.load(f'models/{symbol}_model/scaler_{symbol}.pkl')
